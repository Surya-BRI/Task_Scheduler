...
function FilesPanel() {
    ...
    return (<section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
      ...
      <button type="button" onClick={() => fileInputRef.current?.click()} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-md bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100">
...
export function RetailProjectPage() {
    ...
    const row = useMemo(() => dummyProjects.find((p) => p.id === projectRowId && p.category === 'Retail'), [projectRowId]);
    ...
    useEffect(() => {
        ...
        if (!raw || !RETAIL_TAB_IDS.includes(raw))
            return;
        setActiveTab(raw);
    }, [searchParams, row]);
    useEffect(() => {
      ...
      <main className="h-[calc(100vh-128px)] overflow-auto px-2.5 py-2 pb-3 sm:px-3">
        <div className="mx-auto max-w-[1460px] space-y-1.5">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={() => router.push('/projects-list')} className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-100">
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 pb-1.5">
                <div className="flex items-center gap-4">
                  {TABS.map((tab) => (<TabButton key={tab.id} label={tab.label} active={activeTab === tab.id} onClick={() => selectTab(tab.id)}/>))}
                </div>
              ...
              {activeTab === 'chatter' ? (<div className="mt-3">
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-2.5">
                    ...
                    <textarea id="retail-chatter-input" value={chatterMessage} onChange={(event) => setChatterMessage(event.target.value)} rows={3} placeholder="Type your comment..." className="mt-1.5 w-full resize-none rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/25"/>
