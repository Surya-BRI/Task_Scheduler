...
function StagePill({ item }) {
    ...
    return (<div className="min-w-[148px] rounded-lg border border-slate-200 bg-white px-2 py-1.5">
...
function DetailRow({ label, value }) {
    return (<div className="grid grid-cols-[125px_1fr] gap-2 py-0.5">
      <p className="text-[11px] text-slate-500">{label}</p>
      <p className="text-[13px] font-medium text-slate-900">{value}</p>
    </div>);
}
function TabButton({ active, onClick, label }) {
    return (<button type="button" onClick={onClick} className={`border-b-2 px-1 py-1.5 text-sm ${active
...
function FormFieldWithPencil({ id, label, value, onChange, placeholder }) {
    return (<div>
      ...
      <div className="relative mt-1">
        <input id={id} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-md border border-slate-300 bg-white py-1.5 pl-2.5 pr-9 text-[13px] text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/25"/>
...
function FilesPanel() {
    ...
    return (<section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
      <h2 className="text-sm font-semibold text-slate-900">Files</h2>
      <button type="button" onClick={openFilePicker} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-md bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100">
...
function ProjectDetailsTable() {
    return (<div className="mt-3 overflow-hidden rounded-md border border-slate-200">
      ...
      <div className="max-h-[260px] overflow-auto">
        {PROJECT_TABLE_ROWS.map((row) => (<div key={row.no} className="grid grid-cols-[0.5fr_0.5fr_0.8fr_0.5fr_1fr_0.7fr_0.7fr_0.9fr_0.9fr_0.8fr_0.9fr_0.8fr_0.9fr] items-center border-t border-slate-100 px-2 py-1 text-[11px] text-slate-800">
            <div>{row.tNo}</div>
            <div>{row.no}</div>
...
export function TaskDetailsPage() {
    ...
    useEffect(() => {
        ...
        if (!raw || !TASK_TAB_IDS.includes(raw))
            return;
        if (raw === 'team' && record.designType === 'Retail')
            return;
        setActiveTab(raw);
    }, [searchParams, record]);
    useEffect(() => {
    ...
    const selectTaskTab = useCallback((tabId) => {
        setActiveTab(tabId);
        const next = new URLSearchParams(searchParams.toString());
        if (tabId === 'details') {
            next.delete('tab');
    ...
    const backPath = from === 'project-design'
        ? '/project-design'
        : from === 'projects-list'
            ? '/projects-list'
            : '/design-list';
      ...
      <main className="h-[calc(100vh-128px)] overflow-auto px-2.5 py-2 pb-3 sm:px-3">
        <div className="mx-auto max-w-[1460px] space-y-1.5">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={() => router.push(backPath)} className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-100">
          ...
          <div className="flex gap-2 overflow-x-auto pb-0.5">
            {STAGE_ITEMS.map((item) => (<StagePill key={item.id} item={item}/>))}
          </div>
          ...
          <div className="grid gap-2.5 lg:grid-cols-[1fr_265px]">
            <section className="rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-200 pb-1.5">
                <div className="flex items-center gap-4">
                  {tabs.map((tab) => (<TabButton key={tab.id} label={tab.label} active={activeTab === tab.id} onClick={() => selectTaskTab(tab.id)}/>))}
                </div>
              ...
              {activeTab === 'details' ? (<>
                  ...
                  {isRetail ? (<div className="mt-3 border-t border-slate-200 pt-3">
                      <div className="grid gap-2.5 sm:grid-cols-2">
                        <FormFieldWithPencil id="retail-priority" label="Priority Level" value={priorityLevel} onChange={setPriorityLevel} placeholder=""/>
                        <FormFieldWithPencil id="retail-hours" label="Hours Required" value={hoursRequired} onChange={setHoursRequired} placeholder=""/>
                        <FormFieldWithPencil id="retail-issued" label="Date of Issued" value={dateIssued} onChange={setDateIssued} placeholder=""/>
                        <FormFieldWithPencil id="retail-submission" label="Date of Submission" value={dateSubmission} onChange={setDateSubmission} placeholder=""/>
                      </div>
                      <div className="mt-2.5 flex justify-end">
                        <button type="button" onClick={() => setCreateModalOpen(true)} className="rounded-md bg-[#10a6e3] px-5 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-[#0f96cd] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">
                          Create
                        </button>
                      </div>
                    </div>) : (<div className="mt-3 border-t border-slate-200 pt-3">
                      <div className="grid gap-2.5 sm:grid-cols-2">
                      ...
                      <ProjectDetailsTable />
                      <div className="mt-2.5 flex justify-end">
                        <button type="button" onClick={() => setProjectCreateModalOpen(true)} className="rounded-md bg-[#10a6e3] px-5 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-[#0f96cd] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">
                          Create
                        </button>
                      </div>
                    </div>)}

                  {isRetail ? (<div className="mt-3 overflow-hidden rounded-md border border-slate-200">
              ...
              {activeTab === 'activity' ? (<div className="mt-3 rounded-md border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
                  No activity entries yet.
                </div>) : null}

              {activeTab === 'chatter' ? (<div className="mt-3">
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-2.5">
                    <label htmlFor="chatter-input" className="text-xs font-semibold text-slate-700">
                      Enter a chatter message
                    </label>
                    <textarea id="chatter-input" value={chatterMessage} onChange={(event) => setChatterMessage(event.target.value)} rows={3} placeholder="Type your comment..." className="mt-1.5 w-full resize-none rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/25"/>
                    <div className="mt-1.5 flex justify-end">
                      <button type="button" onClick={handlePostChatter} disabled={!canPostChatter} className="rounded-md bg-[#10a6e3] px-3.5 py-1.5 text-xs font-semibold text-white transition hover:bg-[#0f96cd] disabled:cursor-not-allowed disabled:opacity-60">
                        Post
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 max-h-[210px] space-y-1.5 overflow-auto pr-1">
                    {chatterEntries.length === 0 ? (<div className="rounded-md border border-dashed border-slate-300 bg-white px-4 py-5 text-center text-xs text-slate-500">
                        No chatter messages yet.
                      </div>) : (chatterEntries.map((entry) => (<article key={entry.id} className="rounded-md border border-slate-200 bg-white px-2.5 py-2 text-xs text-slate-800">
                          {entry.text}
                        </article>)))}
                  </div>
                </div>) : null}

              {activeTab === 'team' && !isRetail ? (<div className="mt-3 border-t border-slate-200 pt-3">
                  <div className="grid gap-2.5 sm:grid-cols-2">
            ...
            <aside className="space-y-2.5">
              {activeTab !== 'chatter' ? (<section className="rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm">
                  <h2 className="text-xs font-semibold text-slate-900">Project History</h2>
                  <ul className="mt-2 space-y-1.5 text-xs text-slate-700">
                    {PROJECT_HISTORY.map((entry, idx) => (<li key={entry} className="border-b border-slate-100 pb-1.5 last:border-b-0">
                        <p className="text-[10px] text-slate-500">2026-02-0{idx + 1}</p>
                        <p>{entry}</p>
                      </li>))}
                  </ul>
                </section>) : (<section className="rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm">
                  <h2 className="text-xs font-semibold text-slate-900">Field History</h2>
                  <ul className="mt-2 space-y-2 text-xs text-slate-700">
                    {FIELD_HISTORY.map((entry, idx) => (<li key={entry}>
