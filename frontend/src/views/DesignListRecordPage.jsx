...
function StageChip({ item }) {
    ...
    return (<div className="min-w-[145px] rounded border border-slate-200 bg-white px-2 py-1.5">
...
function RowField({ label, value }) {
    return (<div className="grid grid-cols-[130px_1fr_18px] items-center gap-2 py-0.5">
...
export function DesignListRecordPage() {
    ...
    useEffect(() => {
        ...
        if (!raw || !RECORD_TAB_IDS.includes(raw))
            return;
        setActiveTab(raw);
    }, [searchParams, record]);
    const selectRecordTab = useCallback((tab) => {
        setActiveTab(tab);
    ...
    return (<div className="min-h-screen bg-[#f5f6f8]">
      ...
      <main className="px-3 py-3">
        <div className="mx-auto max-w-[1500px] space-y-3">
          <div>
            <button type="button" onClick={() => router.push('/design-list')} className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-100">
          ...
          <div className="flex gap-2 overflow-x-auto pb-0.5">
            {STAGE_ITEMS.map((item) => (<StageChip key={item.id} item={item}/>))}
          </div>
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_250px]">
            <section className="rounded-lg border border-slate-200 bg-white p-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                <div className="flex items-center gap-4 text-sm">
                  {['details', 'activity', 'chatter'].map((tab) => (<button key={tab} type="button" onClick={() => selectRecordTab(tab)} className={`border-b-2 pb-1 capitalize ${activeTab === tab
              ...
              {activeTab === 'details' ? (<>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input value={providedFile} readOnly className="h-8 flex-1 rounded border border-slate-300 px-2 text-xs"/>
                      <button type="button" onClick={() => fileInputRef.current?.click()} className="h-8 rounded border border-slate-300 px-2 text-xs">
                        Upload
                      </button>
                      <input ref={fileInputRef} type="file" className="hidden" onChange={(event) => setProvidedFile(event.target.files?.[0]?.name ?? providedFile)}/>
                    </div>
                  </div>
              ...
              {activeTab === 'activity' ? (<div className="mt-3 space-y-5">
                  {ACTIVITY_TIMELINE.map((item) => (<div key={item.id} className="grid grid-cols-[22px_1fr] gap-2">
              ...
              {activeTab === 'chatter' ? (<div className="mt-3">
                  <div className="flex items-center justify-between">
                    <button type="button" className="rounded-full bg-slate-100 px-4 py-1.5 text-sm font-medium text-slate-700">
                      Posts
                    </button>
            ...
            <aside className="space-y-3">
              <section className="rounded-lg border border-slate-200 bg-white p-3">
                ...
                {activeTab === 'chatter' ? (<ul className="mt-2 space-y-3 text-xs text-slate-700">
                    {FIELD_HISTORY.map((item) => (<li key={item.id}>
                        <p className="text-slate-500">{item.date}</p>
                        <p>{item.text}</p>
                      </li>))}
                  </ul>) : (<ul className="mt-2 space-y-2 text-xs text-slate-700">
                    <li>
              ...
              <section className="rounded-lg border border-slate-200 bg-white p-3">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} className="mt-2 w-full rounded bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700">
                  Upload Files
                </button>
