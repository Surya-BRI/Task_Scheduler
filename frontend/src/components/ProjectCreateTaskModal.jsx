...
function TableInput({ value, onChange, type = 'text', placeholder = '' }) {
    return (<input value={value} type={type} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="h-6 w-full rounded-full border border-slate-200 bg-slate-50 px-2 text-xs text-slate-700 outline-none focus:border-blue-400"/>);
}
function TickBox({ checked, onChange }) {
    return (<input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-4 w-4 rounded border-slate-300 text-blue-600"/>);
}
export function ProjectCreateTaskModal({ open, onClose }) {
    ...
    function rowHasSelection(r) {
        const hasFlag = r.artwork || r.technical || r.location || r.asBuilt || r.bim;
        const hasHours = [r.artHours, r.techHours, r.locationHours, r.asBuiltHours].some((h) => String(h ?? '').trim() !== '');
    ...
    return (<div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-900/30 p-4 pt-16">
      ...
      <div className="relative z-10 w-full max-w-[1200px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
        ...
        <div className="space-y-3 p-4">
          <div className="grid grid-cols-[1fr_1fr_1fr_1fr_auto_auto] gap-4">
            <select value={selectedSignType} onChange={(event) => setSelectedSignType(event.target.value)} className="h-10 rounded-md border border-slate-300 px-3 text-sm">
              <option>Select sign type</option>
              {rows.map((row) => (<option key={row.id} value={row.signType}>
                  {row.signType}
                </option>))}
            </select>
            <input value={planCode} onChange={(event) => setPlanCode(event.target.value)} className="h-10 rounded-md border border-slate-300 px-3 text-sm" placeholder="Enter Plan Code"/>
            <select value={selectedArea} onChange={(event) => setSelectedArea(event.target.value)} className="h-10 rounded-md border border-slate-300 px-3 text-sm">
              <option value="">Select Area</option>
              {AREA_OPTIONS.map((area) => (<option key={area} value={area}>
                  {area}
                </option>))}
            </select>
            <select value={selectedLevel} onChange={(event) => setSelectedLevel(event.target.value)} className="h-10 rounded-md border border-slate-300 px-3 text-sm">
              <option value="">Select Level</option>
              {LEVEL_OPTIONS.map((level) => (<option key={level} value={level}>
                  {level}
                </option>))}
            </select>
          ...
          <div className="overflow-hidden rounded-lg border border-slate-200">
            ...
            <div className="max-h-[360px] overflow-auto">
              {rows.map((row) => (<div key={row.id} className="border-b border-slate-200">
                  ...
                  {expanded.has(row.id) &&
                row.children.map((child) => (<div key={child.id} className="grid grid-cols-[1.2fr_0.9fr_0.8fr_0.9fr_0.8fr_0.9fr_0.8fr_0.9fr_0.8fr_0.8fr_1.1fr] items-center border-t border-slate-100 bg-slate-50/80 text-xs">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button type="button" className="rounded-md bg-[#1f3b68] px-8 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#163056] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1f3b68] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-45" disabled={selectedCount === 0} title={selectedCount === 0 ? 'Select at least one work type or enter hours on a row' : undefined}>
              Create Tasks
            </button>
          </div>
        </div>
