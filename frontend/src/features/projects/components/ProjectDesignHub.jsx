...
function ActionLink({ href, label }) {
    return (<Link href={href} className="inline-flex rounded border border-gray-200 bg-white px-2 py-1 text-[11px] font-medium text-gray-700 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-900">
      {label}
    </Link>);
}
        ...
        <table className="w-full text-xs text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.length === 0 ? (<tr>
                <td colSpan={5} className="px-3 py-8 text-center text-gray-500 text-sm">
              ...
              </tr>) : (rows.map((row) => (<tr key={row.id} className="hover:bg-gray-50">
                  <td className="px-2 py-2">
                    <Link href={hubTaskUrl(row.id)} className="font-medium text-blue-600 hover:underline whitespace-nowrap">
                      {row.opNo}
                    </Link>
                  </td>
                  <td className="px-2 py-2">
                    <Link href={hubTaskUrl(row.id)} className="text-blue-600 hover:underline whitespace-nowrap">
                      {row.projectNo}
                    </Link>
                  </td>
                  ...
                  <td className="px-2 py-2">
                    <div className="flex flex-wrap items-center justify-center gap-1.5">
                      ...
                      {variant === "project" ? (<ActionLink href={hubTaskUrl(row.id, { tab: "team" })} label="Team"/>) : null}
...
export function ProjectDesignHub() {
    ...
    const tabClass = (active) => `rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${active
      ...
      <div className="flex-1 flex flex-col min-h-0 px-6 pb-6">
        <div className="shrink-0 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-6 mb-4">
          ...
          <div className="relative max-w-md">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search OP, project no, name…" className="pl-9 pr-4 py-2 border border-gray-200 rounded-md text-sm w-full sm:w-72 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"/>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          {segment === "retail" ? (<DesignTypeTable rows={retailRows} variant="retail"/>) : (<DesignTypeTable rows={projectRows} variant="project"/>)}
        </div>
      </div>
    </div>);
}
