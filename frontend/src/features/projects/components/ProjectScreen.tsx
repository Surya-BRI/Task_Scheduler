...
function resolveTaskIdForProjectRow(row: ProjectEntry, records: DesignRec[]): string | null {
...
function projectListTaskHref(taskId: string) {
  return `/design-list/task/${encodeURIComponent(taskId)}?from=projects-list`;
}

function retailProjectHref(projectRowId: string) {
...
// ─── Table ─────────────────────────────────────────────────────────────────────
function ProjectTable({ data, records }: { data: ProjectEntry[]; records: DesignRec[] }) {
  return (
...
export function ProjectScreen() {
  ...
  const designRecords = records as unknown as DesignRec[];
