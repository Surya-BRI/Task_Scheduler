...
function recordDetailPath(record, workflowFrom) {
  return taskViewPathForRecord(record, { from: workflowFrom });
}

function recordTabPath(record, tab, workflowFrom) {
  return taskSummaryPath(record?.id, { from: workflowFrom, tab });
}
...
const Table = ({ data, workflowFrom }) => {
  const router = useRouter();
  return (
    <div className="flex min-h-0 flex-1 flex-col px-4 pb-4 sm:px-6"><div className="ui-surface flex min-h-0 flex-1 flex-col overflow-hidden"><div className="min-h-0 flex-1 overflow-auto"><table className="w-full min-w-[1100px] border-collapse text-left text-[11px] text-slate-700"><thead className="sticky top-0 z-10 border-b border-slate-200 bg-slate-100"><tr><th className="px-2 py-1.5">OP No</th><th className="px-2 py-1.5">Project No</th><th className="px-2 py-1.5">Business Unit</th><th className="px-2 py-1.5">Design Type</th><th className="px-2 py-1.5">Revision</th><th className="px-2 py-1.5">Project Name</th><th className="px-2 py-1.5">Status</th><th className="px-2 py-1.5">Sales Person</th><th className="px-2 py-1.5">Created</th><th className="px-2 py-1.5">Deadline</th><th className="px-2 py-1.5 text-right">Aging</th><th className="px-2 py-1.5 text-center">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{data.map((row, index) => { const overdue = isOverdue(row); return (<tr key={`${row.id}-${index}`} className={`transition-colors hover:bg-blue-50/40 ${overdue ? "bg-red-50/40" : index % 2 === 0 ? "bg-white" : "bg-slate-50/60"}`}><td className="px-2 py-1.5 text-xs font-medium text-slate-800">{row.opNo || "�"}</td><td className="px-2 py-1.5"><button type="button" onClick={() => router.push(recordDetailPath(row, workflowFrom))} className="text-left text-xs font-medium text-blue-600 hover:text-blue-700 hover:underline">{row.projectNo || "�"}</button></td><td className="px-2 py-1.5">{row.businessUnit || "�"}</td><td className="px-2 py-1.5 font-medium text-slate-800 whitespace-nowrap" ><TypeOfDesignChip value={row.typeOfDesign} /></td><td className="px-2 py-1.5">{row.revisionCode}</td><td className="px-2 py-1.5 w-[200px] whitespace-nowrap" title={row.projectName || "�"}>{truncateText(row.projectName, 20)}</td><td className="px-2 py-1.5"><span className={`inline-flex items-center rounded-full border px-1.5 py-px text-[9px] font-semibold leading-tight ${getStatusColor(row.status)}`}>{getStatusLabel(row.status)}</span></td><td className="px-2 py-1.5">{row.salesPerson || "Unassigned"}</td><td className="px-2 py-1.5 whitespace-nowrap">{row.created || "�"}</td><td className={`px-2 py-1.5 whitespace-nowrap font-medium ${overdue ? "text-red-600" : "text-slate-700"}`}>{overdue ? <span className="inline-flex items-center gap-1"><span className="inline-block h-1.5 w-1.5 rounded-full bg-red-500 shrink-0" />{row.deadline}</span> : (row.deadline || "�")}</td><td className={`px-2 py-1.5 whitespace-nowrap text-right font-semibold ${row.agingDays > 20 ? "text-red-600" : "text-slate-600"}`}>{row.agingDays != null ? `${row.agingDays} d` : "�"}</td><td className="px-2 py-1.5"><div className="flex items-center justify-center gap-1.5 text-slate-400"><button type="button" onClick={() => router.push(recordDetailPath(row, workflowFrom))} className="rounded p-0.5 transition-colors hover:bg-slate-100 hover:text-blue-600" title="View details"><Eye className="h-3.5 w-3.5" /></button><button type="button" onClick={() => router.push(recordTabPath(row, "chatter", workflowFrom))} className="rounded p-0.5 transition-colors hover:bg-slate-100 hover:text-slate-600" title="Links & chatter"><Link2 className="h-3.5 w-3.5" /></button><button type="button" onClick={() => router.push(recordTabPath(row, "activity", workflowFrom))} className="rounded p-0.5 transition-colors hover:bg-slate-100 hover:text-emerald-600" title="Activity history"><History className="h-3.5 w-3.5" /></button><button type="button" onClick={() => router.push(recordDetailPath(row, workflowFrom))} className="rounded p-0.5 transition-colors hover:bg-slate-100 hover:text-violet-600" title="Assignee"><UserRound className="h-3.5 w-3.5" /></button></div></td></tr>); })}</tbody></table></div></div></div>
  );
};
              ...
              >
                <div className="text-[10px] border-b border-slate-200/50 pb-1 mb-1 whitespace-nowrap overflow-hidden text-ellipsis">
                  ...
                  {item.typeOfDesign && item.typeOfDesign !== "—" ? (
                    <>
                      {" | "}
                      <TypeOfDesignChip value={item.typeOfDesign} />
                    </>
                  ) : null}
                </div>
