...
const Toolbar = ({ viewMode, setViewMode, filters, setFilters, designerName }) => {
  const [showFilters, setShowFilters] = useState(false);
  const [typeMenuOpen, setTypeMenuOpen] = useState(false);
  const filtersPanelRef = useRef(null);
  const activeCount = [filters.type, filters.status, filters.startDate, filters.endDate].filter(Boolean).length;
  const selectedTypeLabel = TYPE_OPTIONS.find((o) => o.value === filters.type)?.label ?? "All Types";

  function clearFilters() {
    setFilters({ type: "", status: "", startDate: "", endDate: "", searchQuery: filters.searchQuery });
  }
    ...
    <div className="mb-4 mt-4 flex flex-col gap-4 px-4 sm:px-6 md:flex-row md:items-center md:justify-between">
      ...
      <div className="relative flex flex-wrap items-center justify-end gap-2 sm:gap-3 md:ml-auto">
          ...
          {showFilters && (
            <div className="ui-surface absolute right-0 top-11 z-50 flex w-[340px] flex-col gap-4 p-5 shadow-lg">
              <div className="mb-2 flex items-center justify-between border-b border-slate-200 pb-2">
                ...
                {activeCount > 0 && (
                  <button type="button" onClick={clearFilters} className="cursor-pointer rounded bg-red-50 px-2 py-1 text-xs font-medium text-red-500 transition-colors hover:text-red-700">
                    Clear All
                  </button>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1.5 mt-2">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors cursor-pointer ${filters.status === "" ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}`}
                  >
                    All
                  </button>
                  {DESIGNER_QUEUE_FILTER_STATUSES.map((status) => (
                    <button
                      type="button"
                      key={status}
                      onClick={() => setFilters({ ...filters, status })}
                      className={`px-3 py-1 rounded-full text-xs font-medium border focus:outline-none transition-all cursor-pointer ${filters.status === status ? `ring-2 ring-blue-500 ring-offset-1 shadow-sm ${getStatusColor(status)}` : `bg-white ${getStatusColor(status).replace("bg-", "hover:bg-").split(" ").filter((c) => !c.startsWith("bg-")).join(" ")}`}`}
                    >
        ...
        <div className="ml-0 flex rounded-md border border-slate-200 bg-slate-100 p-1 sm:ml-1">
          <button type="button" onClick={() => setViewMode("list")} title="List View" className={`p-1.5 rounded transition-colors cursor-pointer ${viewMode === "list" ? "bg-white shadow text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
            <List size={16} />
          </button>
          <button type="button" onClick={() => setViewMode("board")} title="Board View" className={`p-1.5 rounded transition-colors cursor-pointer ${viewMode === "board" ? "bg-white shadow text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
            <LayoutGrid size={16} />
          </button>
        </div>
      </div>
