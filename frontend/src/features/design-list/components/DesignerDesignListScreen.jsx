...
import { Calendar, Clock, Filter, GalleryVerticalEnd, LayoutGrid, List, Pause, Play, Search, Square } from "lucide-react";
    ...
    <div className="mb-4 mt-4 flex flex-col gap-4 px-4 sm:px-6 md:flex-row md:items-center md:justify-between">
      ...
      <div className="relative flex flex-wrap items-center justify-end gap-2 sm:gap-3 md:ml-auto">
        ...
        {showFilters && (
          <div className="ui-surface absolute right-20 top-12 z-50 flex w-[340px] flex-col gap-4 p-5">
            <div className="mb-2 flex items-center justify-between border-b border-slate-200 pb-2">
              ...
              {activeCount > 0 && (
                <button onClick={clearFilters} className="cursor-pointer rounded bg-red-50 px-2 py-1 text-xs font-medium text-red-500 transition-colors hover:text-red-700">
                  Clear All
                </button>
              )}
            </div>
        ...
        <div className="ml-0 flex rounded-md border border-slate-200 bg-slate-100 p-1 sm:ml-1">
          <button onClick={() => setViewMode("list")} title="List View" className={`p-1.5 rounded transition-colors cursor-pointer ${viewMode === "list" ? "bg-white shadow text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
            <List size={16} />
          </button>
          <button onClick={() => setViewMode("board")} title="Board View" className={`p-1.5 rounded transition-colors cursor-pointer ${viewMode === "board" ? "bg-white shadow text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
            <LayoutGrid size={16} />
          </button>
        </div>
      </div>
