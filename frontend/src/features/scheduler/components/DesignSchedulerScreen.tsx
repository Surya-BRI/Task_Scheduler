...
const INITIAL_SCHEDULES: Record<string, Record<string, string[]>> = {
...
export function DesignSchedulerScreen() {
  ...
  const handleDragStart = (e: React.DragEvent, taskId: string, sourceId: string, sourceDay?: string) => {
    ...
    if (sourceDay) e.dataTransfer.setData("sourceDay", sourceDay);
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left Sidebar */}
        <div 
          className="w-64 bg-[#f8f9fc] border-r border-gray-200 flex flex-col shrink-0"
          onDragOver={handleDragOver}
          onDrop={(e) => handleDropToPanel(e, "unassigned")}
        >
          <div className="p-4 flex flex-col h-full">
             ...
             <div className="relative mb-6">
                ...
                <input 
                  ...
                  className="w-full bg-white border border-gray-200 shadow-sm rounded-md py-1.5 pl-8 pr-3 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
             </div>
                      ...
                      <div className="flex-1 grid grid-cols-7 relative">
                        {[0, 1, 2, 3, 4, 5, 6].map(dayIndex => {
                          ...
                          return (
                            <div 
                              key={dayIndex}
                              className={`border-r relative flex flex-col transition-colors overflow-hidden
                                ${isWeekend 
                              ...
                              `}
                              onDragOver={isWeekend ? undefined : handleDragOver}
                              onDrop={isWeekend ? undefined : (e) => handleDropToDay(e, designer.id, dayIndex)}
                            >
                              {/* Gravity fill bar (background) — weekdays only */}
                              {!isWeekend && dayHours > 0 && (
                                <div 
                                  className={`absolute bottom-0 left-0 right-0 transition-all opacity-20 ${isDayOverloaded ? 'bg-red-400' : 'bg-blue-400'}`}
                              ...
                              {!isWeekend && dayHours > 0 && (
                                <div className={`text-[8px] font-bold text-center pb-0.5 relative z-10 ${isDayOverloaded ? 'text-red-600' : 'text-blue-500/70'}`}>
